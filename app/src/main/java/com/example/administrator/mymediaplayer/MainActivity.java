...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //加载但前布局文件控件操作
        tiele = (TextView) findViewById(R.id.title);
        ...
        holder.addCallback(new SurfaceHolder.Callback() {
            ...
            public void surfaceCreated(SurfaceHolder holder) {
                //当surfaceCreated中的被创建的时候被调用
                //在这里我们指定MediaPlayer在当前Surface中进行播放
                player.setDisplay(holder);
                //在指定MediaPlayer播放的容器后，我们就可以使用prepare或者prepareAsync来准备播放了
                player.prepareAsync();
            }
            @Override
        ...
        //为了可以播放视频或使用Camera预览，我们需要指定Buffer类型
        holder.setType(SurfaceHolder.SURFACE_TYPE_PUSH_BUFFERS);
        //下面开始实例话MediaPlayer对象
        ...
        player.setOnPreparedListener(new MediaPlayer.OnPreparedListener() {
            ...
            public void onPrepared(MediaPlayer mp) {
                ...
                if (vWidth > currDisplay.getWidth() || vHeight > currDisplay.getHeight()) {
                    //如果video的宽或高超出当前屏幕的大小，则要进行缩放
                    float wRation = (float) vWidth / (float) currDisplay.getWidth();
                    float hRation = (float) vWidth / (float) currDisplay.getHeight();
                    //选择大的一个进行缩放
                    float ratio = Math.max(wRation, hRation);
        ...
        try {
            ...
            if (!mPath.equals("")) {
                tiele.setText(mPath.substring(mPath.lastIndexOf("/") + 1));
